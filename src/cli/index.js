...
if (
  ...
  help({
    usage: [
      'segmentdesign [--input input.css] [--output output.css] [--watch] [options...]',
      'segmentdesign init [--full] [--postcss] [options...]',
    ],
...
if (commands[command] === undefined) {
  if (fs.existsSync(path.resolve(command))) {
    ...
    help({
      ...
      usage: ['segmentdesign <command> [options]'],
      commands: Object.keys(commands)
        .filter((command) => command !== 'build')
        .map((command) => `${command} [options]`),
      options: sharedFlags,
...
let args = (() => {
  try {
    ...
    //
    // E.g.: segmentdesign input.css -> _: ['build', 'input.css']
    // E.g.: segmentdesign build input.css -> _: ['build', 'input.css']
