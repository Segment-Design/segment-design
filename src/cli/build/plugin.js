...
import segmentdesign from "../../processsegmentdesignFeatures";
import {
  loadAutoprefixer,
  loadCssNano,
  loadPostcss,
...
async function loadPostCssPlugins(customPostCssPath) {
  ...
  let configPluginsegmentdesignIdx = configPlugins.findIndex((plugin) => {
    if (typeof plugin === "function" && plugin.name === "segmentdesign") {
      return true;
    }
    ...
    if (
      ...
      plugin !== null &&
      plugin.postcssPlugin === "segmentdesign"
    ) {
      return true;
    }
  ...
  let beforePlugins =
    configPluginsegmentdesignIdx === -1
      ? []
      : configPlugins.slice(0, configPluginsegmentdesignIdx);
  let afterPlugins =
    configPluginsegmentdesignIdx === -1
      ? configPlugins
      : configPlugins.slice(configPluginsegmentdesignIdx + 1);
...
function loadBuiltinPostcssPlugins() {
  ...
  let IMPORT_COMMENT = "__segmentdesign_RESTORE_IMPORT__: ";
  return [
    [
      (root) => {
        root.walkAtRules("import", (rule) => {
          if (rule.params.slice(1).startsWith("segmentdesign/")) {
...
let state = {
  ...
  refreshContentPaths() {
    this.contentPaths = parseCandidateFiles(
      this.context,
      this.context?.segmentdesignConfig,
    );
  },

  get config() {
    return this.context.segmentdesignConfig;
  },
  ...
  readContentPaths() {
    ...
    // Resolve raw content in the segmentdesign config
...
export async function createProcessor(args, cliConfigPath) {
  ...
  let customPostCssPath =
    typeof args["--postcss"] === "string" ? args["--postcss"] : undefined;
  ...
  let segmentdesignPlugin = () => {
    return {
      postcssPlugin: "segmentdesign",
      async Once(root, { result }) {
        env.DEBUG && console.time("Compiling CSS");
        await segmentdesign(({ createContext }) => {
          console.error();
  ...
  segmentdesignPlugin.postcss = true;
  ...
  async function readInput() {
    ...
    return "@segmentdesign base; @segmentdesign components; @segmentdesign utilities";
  }
  ...
  async function build() {
    ...
    return readInput()
      .then((css) =>
        processor.process(css, { ...postcssOptions, from: input, to: output }),
      )
      .then((result) => {
        if (!state.watcher) {
          return result;
      ...
      .then((result) => {
        ...
        return Promise.all([
          ...
          result.map &&
            outputFile(result.opts.to + ".map", result.map.toString()),
        ]);
      })
      .then(() => {
