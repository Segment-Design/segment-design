...
import { version as segmentDesignVersion } from "../package.json";
  ...
  pseudoElementVariants: ({ addVariant }) => {
    ...
    addVariant("before", ({ container }) => {
      container.walkRules((rule) => {
        ...
        if (!foundContent) {
          rule.prepend(
            postcss.decl({ prop: "content", value: "var(--tw-content)" }),
          );
        }
      });
  ...
  darkVariants: ({ config, addVariant }) => {
    ...
    if (mode === false) {
      ...
      log.warn("darkmode-false", [
        "The `darkMode` option in your segmentdesign configuration is set to `false`, which now behaves the same as `media`.",
        "Change `darkMode` to `media` or remove it entirely.",
        "https://segmentdesign.com/docs/upgrade-guide#remove-dark-mode-configuration",
      ]);
    }
  ...
  screenVariants: ({ theme, addVariant, matchVariant }) => {
    ...
    for (let screen of screens) {
      addVariant(screen.name, `@media ${buildMediaQuery(screen)}`, {
        id,
        sort:
          areSimpleScreens && screensUseConsistentUnits ? minSort : undefined,
        value: screen,
      });
    }
...
export let corePlugins = {
  preflight: ({ addBase }) => {
    ...
    addBase([
      postcss.comment({
        text: `! segmentdesign v${segmentDesignVersion} | MIT License | https://segmentdesign.com`,
      }),
  ...
  container: (() => {
    function extractMinWidths(breakpoints = []) {
      return breakpoints
        .flatMap((breakpoint) =>
          breakpoint.values.map((breakpoint) => breakpoint.min),
        )
  ...
  order: createUtilityPlugin("order", undefined, {
    supportsNegativeValues: true,
  }),
