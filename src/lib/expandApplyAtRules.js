...
function buildLocalApplyCache(root, context) {
  ...
  root.walkRules((rule) => {
    // Ignore rules generated by segmentdesign
    for (let node of pathToRoot(rule)) {
      if (node.raws.segmentdesign?.layer !== undefined) {
        return
      }
    }
  ...
  for (let [parent, [candidates, atApplySource]] of perParentApplies) {
    ...
    for (let [applyCandidate, important, rules] of candidates) {
      let potentialApplyCandidates = [
        applyCandidate,
        ...extractBaseCandidates([applyCandidate], context.segmentdesignConfig.separator),
      ]
      ...
      for (let [meta, node] of rules) {
        ...
        nodeClasses = nodeClasses.concat(
          extractBaseCandidates(nodeClasses, context.segmentdesignConfig.separator)
        )
        ...
        if (canRewriteSelector) {
          root.walkRules((rule) => {
            ...
            let importantSelector =
              typeof context.segmentdesignConfig.important === 'string'
                ? context.segmentdesignConfig.important
                : null

            // We only want to move the "important" selector if this is a segmentdesign-generated utility
            // We do *not* want to do this for user CSS that happens to be structured the same
            let isGenerated = parent.raws.segmentdesign !== undefined
