...
function getCandidateFiles(context, segmentdesignConfig) {
  if (candidateFilesCache.has(context)) {
    return candidateFilesCache.get(context)
  }

  let candidateFiles = parseCandidateFiles(context, segmentdesignConfig)

  return candidateFilesCache.set(context, candidateFiles).get(context)
}

// Get the config object based on a path
function getsegmentdesignConfig(configOrPath) {
...
export default function setupTrackingContext(configOrPath) {
  return ({ segmentdesignDirectives, registerDependency }) => {
    return (root, result) => {
      let [segmentdesignConfig, userConfigPath, segmentdesignConfigHash, configDependencies] =
        getsegmentdesignConfig(configOrPath)

      let contextDependencies = new Set(configDependencies)

      // If there are no @segmentdesign or @apply rules, we don't consider this CSS
      // file or its dependencies to be dependencies of the context. Can reuse
      // the context even if they change. We may want to think about `@layer`
      // being part of this trigger too, but it's tough because it's impossible
      // for a layer in one file to end up in the actual @segmentdesign rule in
      // another file since independent sources are effectively isolated.
      if (segmentdesignDirectives.size > 0) {
      ...
      let candidateFiles = getCandidateFiles(context, segmentdesignConfig)

      // If there are no @segmentdesign or @apply rules, we don't consider this CSS file or it's
      // dependencies to be dependencies of the context. Can reuse the context even if they change.
      // We may want to think about `@layer` being part of this trigger too, but it's tough
      // because it's impossible for a layer in one file to end up in the actual @segmentdesign rule
