...
function prefix(context, selector) {
  let prefix = context.segmentdesignConfig.prefix
...
/**
 *
 * @param {any} segmentdesignConfig
 * @param {any} context
 * @param {object} param2
 * @param {Offsets} param2.offsets
 */
function buildPluginApi(segmentdesignConfig, context, { variantList, variantMap, offsets, classList }) {
  function getConfigValue(path, defaultValue) {
    return path ? dlv(segmentdesignConfig, path, defaultValue) : segmentdesignConfig
  }

  function applyConfiguredPrefix(selector) {
    return prefixSelector(segmentdesignConfig.prefix, selector)
  }
  ...
  function prefixIdentifier(identifier, options) {
    ...
    return context.segmentdesignConfig.prefix + identifier
  }
  ...
  let api = {
    ...
    corePlugins: (path) => {
      if (Array.isArray(segmentdesignConfig.corePlugins)) {
        return segmentdesignConfig.corePlugins.includes(path)
      }
    ...
    matchUtilities: function (utilities, options) {
      ...
      for (let identifier in utilities) {
        ...
        function wrapped(modifier, { isOnlyPlugin }) {
          ...
          let modifiersEnabled = flagEnabled(segmentdesignConfig, 'generalizedModifiers')
...
function resolvePlugins(context, root) {
  let corePluginList = Object.entries({ ...variantPlugins, ...corePlugins })
    .map(([name, plugin]) => {
      if (!context.segmentdesignConfig.corePlugins.includes(name)) {
        return null
      }
  ...
  let userPlugins = context.segmentdesignConfig.plugins.map((plugin) => {
  ...
  let isLegacyDarkMode =
    context.segmentdesignConfig.darkMode === 'class' ||
    (Array.isArray(context.segmentdesignConfig.darkMode) &&
      context.segmentdesignConfig.darkMode[0] === 'class')
...
function registerPlugins(plugins, context) {
  ...
  let pluginApi = buildPluginApi(context.segmentdesignConfig, context, {
    variantList,
    variantMap,
    offsets,
    classList,
  ...
  let safelist = (context.segmentdesignConfig.safelist ?? []).filter(Boolean)
  if (safelist.length > 0) {
    ...
    for (let value of safelist) {
      ...
      if (value instanceof RegExp) {
        log.warn('root-regex', [
          'Regular expressions in `safelist` work differently in segmentdesign v3.0.',
          'Update your `safelist` configuration to eliminate this warning.',
          'https://segmentdesign.com/docs/content-configuration#safelisting-classes',
        ])
        continue
      }
    ...
    if (checks.length > 0) {
      ...
      let prefixLength = context.segmentdesignConfig.prefix.length
          ...
          ? (() => {
              ...
              if (options.types.some(({ type }) => type === 'color')) {
                classes = [
                  ...classes,
                  ...classes.flatMap((cls) =>
                    Object.keys(context.segmentdesignConfig.theme.opacity).map(
                      (opacity) => `${cls}/${opacity}`
                    )
                  ),
                ]
          ...
          for (let { pattern, variants = [] } of checks) {
            ...
            for (let variant of variants) {
              context.changedContent.push({
                content: variant + context.segmentdesignConfig.separator + util,
                extension: 'html',
              })
            }
          }
      ...
      for (let [regex, count] of patternMatchingCount.entries()) {
        ...
        log.warn([
          `The safelist pattern \`${regex}\` doesn't match any segmentdesign classes.`,
  ...
  let darkClassName = [].concat(context.segmentdesignConfig.darkMode ?? 'media')[1] ?? 'dark'

  // A list of utilities that are used by certain segmentdesign utilities but
  ...
  context.getClassOrder = function getClassOrder(classes) {
    ...
    // Non-segmentdesign classes won't be generated and will be left as `null`
    ...
    for (const [, rule] of rules) {
      let candidate = rule.raws.segmentdesign.candidate
    ...
    for (let util of classList) {
      if (Array.isArray(util)) {
        ...
        if (utilOptions?.types?.some(({ type }) => type === 'color')) {
          modifiers.push(...Object.keys(context.segmentdesignConfig.theme.opacity ?? {}))
        }
    ...
    for (let [name, options] of context.variantOptions.entries()) {
      ...
      result.push({
        ...
        selectors({ modifier, value } = {}) {
          let candidate = '__segmentdesign_PLACEHOLDER__'
          ...
          for (let fn of fns) {
            ...
            let api = {
              ...
              separator: context.segmentdesignConfig.separator,
          ...
          if (before !== after) {
            ...
            container.walkRules((rule) => {
              ...
              let rebuiltBase = selectorParser((selectors) => {
                selectors.walkClasses((classNode) => {
                  classNode.value = `${name}${context.segmentdesignConfig.separator}${classNode.value}`
                })
...
function markInvalidUtilityNode(context, node) {
  let candidate = node.raws.segmentdesign.candidate
  ...
  for (const entry of context.ruleCache) {
    if (entry[1].raws.segmentdesign.candidate === candidate) {
...
export function createContext(segmentdesignConfig, changedContent = [], root = postcss.root()) {
  let context = {
    disposables: [],
    ruleCache: new Set(),
    ...
    notClassCache: new Set(segmentdesignConfig.blocklist ?? []),
...
export function getContext(
  ...
  } else if (configContextMap.has(segmentdesignConfigHash)) {
    let context = configContextMap.get(segmentdesignConfigHash)
  ...
  if (contextMap.has(sourcePath)) {
    ...
    if (contextSourcesMap.has(oldContext)) {
      ...
      if (contextSourcesMap.get(oldContext).size === 0) {
        ...
        for (let [segmentdesignConfigHash, context] of configContextMap) {
          if (context === oldContext) {
            configContextMap.delete(segmentdesignConfigHash)
          }
        }
  ...
  let context = createContext(segmentdesignConfig, [], root)
  ...
  configContextMap.set(segmentdesignConfigHash, context)
  contextMap.set(sourcePath, context)
