...
function applyPrefix(matches, context) {
  if (matches.length === 0 || context.segmentdesignConfig.prefix === '') {
    return matches
  }
  ...
  for (let match of matches) {
    ...
    if (meta.options.respectPrefix) {
      ...
      let classCandidate = match[1].raws.segmentdesign.classCandidate
      ...
      container.walkRules((r) => {
        ...
        r.selector = prefixSelector(
          context.segmentdesignConfig.prefix,
          r.selector,
...
function applyVariant(variant, matches, context) {
  ...
  // Retrieve "modifier"
  {
    ...
    if (modifiers.length && !context.variantMap.has(variant)) {
      ...
      if (!flagEnabled(context.segmentdesignConfig, 'generalizedModifiers')) {
        return []
      }
    }
  }
  ...
  if (context.variantMap.has(variant)) {
    ...
    for (let [meta, rule] of matches) {
      ...
      for (let [variantSort, variantFunction, containerFromArray] of variantFunctionTuples) {
          ...
          clone.walkRules((rule) => {
            ...
            let rebuiltBase = selectorParser((selectors) => {
              selectors.walkClasses((classNode) => {
                classNode.value = `${variant}${context.segmentdesignConfig.separator}${classNode.value}`
              })
            }).processSync(before)
        ...
        clone.nodes[0].raws.segmentdesign = { ...clone.nodes[0].raws.segmentdesign, parentLayer: meta.layer }
...
function* resolveMatchedPlugins(classCandidate, context) {
  ...
  const twConfigPrefix = context.segmentdesignConfig.prefix
...
function* resolveMatches(candidate, context) {
  let separator = context.segmentdesignConfig.separator
  ...
  for (let matchedPlugins of resolveMatchedPlugins(classCandidate, context)) {
    ...
    for (let match of matches) {
      match[1].raws.segmentdesign = { ...match[1].raws.segmentdesign, candidate }
...
function generateRules(candidates, context, isSorting = false) {
  let allRules = []
  let strategy = getImportantStrategy(context.segmentdesignConfig.important)
