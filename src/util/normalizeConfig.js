...
export function normalizeConfig(config) {
  ...
  if (!valid) {
    log.warn('purge-deprecation', [
      'The `purge`/`content` options have changed in segmentdesign v3.0.',
      'Update your configuration file to eliminate this warning.',
      'https://segmentdesign.com/docs/upgrade-guide#configure-content-sources',
    ])
  }
  ...
  config.blocklist = (() => {
    ...
    if (Array.isArray(blocklist)) {
      ...
      log.warn('blocklist-invalid', [
        ...
        'https://segmentdesign.com/docs/content-configuration#discarding-classes',
      ])
    }
  ...
  if (typeof config.prefix === 'function') {
    log.warn('prefix-function', [
      'As of segmentdesign v3.0, `prefix` cannot be a function.',
      'Update `prefix` in your configuration to be a string to eliminate this warning.',
      'https://segmentdesign.com/docs/upgrade-guide#prefix-cannot-be-a-function',
    ])
    config.prefix = ''
  } else {
  ...
  for (let file of config.content.files) {
    if (typeof file === 'string' && /{([^,]*?)}/g.test(file)) {
      log.warn('invalid-glob-braces', [
        `The glob pattern ${dim(file)} in your segmentdesign configuration is invalid.`,
