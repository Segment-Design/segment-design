...
async function loadPostCssPlugins(customPostCssPath) {
  ...
  let configPluginsegmentdesignIdx = configPlugins.findIndex((plugin) => {
    ...
    if (typeof plugin === 'object' && plugin !== null && plugin.postcssPlugin === 'segmentdesign') {
      return true
    }
  ...
  let beforePlugins =
    configPluginsegmentdesignIdx === -1 ? [] : configPlugins.slice(0, configPluginsegmentdesignIdx)
  let afterPlugins =
...
let state = {
  ...
  refreshContentPaths() {
    this.contentPaths = parseCandidateFiles(this.context, this.context?.segmentdesignConfig)
  },
