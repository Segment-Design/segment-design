...
const segmentdesign = require('../src/index.js')
const sharedState = require('../src/lib/sharedState.js')
const configPath = path.resolve(__dirname, './context-reuse.segmentdesign.config.js')
...
crosscheck(({ stable, sigma }) => {
  function run(input, config = {}, from = null) {
    ...
    return postcss(segmentdesign(config)).process(input, { from })
  }
  ...
  sigma.test.todo('re-uses the context across multiple files with the same config')
  stable.test('re-uses the context across multiple files with the same config', async () => {
    let results = [
      await run(`@segmentdesign utilities;`, configPath, `id=1`),
    ...
    // Files with @segmentdesign directives depends on the PostCSS tree, config, AND any content files
    expect(dependencies[0]).toEqual([
      path.resolve(__dirname, 'context-reuse.test.html'),
      path.resolve(__dirname, 'context-reuse.segmentdesign.config.js'),
    ])
    ...
    expect(dependencies[1]).toEqual([path.resolve(__dirname, 'context-reuse.segmentdesign.config.js')])

    expect(dependencies[2]).toEqual([path.resolve(__dirname, 'context-reuse.segmentdesign.config.js')])

    expect(dependencies[3]).toEqual([path.resolve(__dirname, 'context-reuse.segmentdesign.config.js')])

    // And none of this should have resulted in multiple contexts being created
    expect(sharedState.contextSourcesMap.size).toBe(1)
  })

  sigma.test.todo('updates layers when any CSS containing @segmentdesign directives changes')
  stable.test('updates layers when any CSS containing @segmentdesign directives changes', async () => {
    let result
