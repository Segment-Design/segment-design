...
crosscheck(({ stable, sigma }) => {
  test('variants', () => {
    ...
    return run(input, config).then((result) => {
      ...
      sigma
        .expect(result.css)
        .toMatchFormattedCss(
          fs.readFileSync(path.resolve(__dirname, './variants.sigma.test.css'), 'utf8')
        )
    })
  })
  ...
  describe('custom advanced variants', () => {
    test('at-rules without params', () => {
      ...
      return run('@segmentdesign components; @segmentdesign utilities', config).then((result) => {
    ...
    test('variant format string must include at-rule or & (1)', async () => {
      ...
      await expect(run('@segmentdesign components;@segmentdesign utilities', config)).rejects.toThrowError(
  ...
  test('stacked peer variants', async () => {
    ...
    `)
    sigma.expect(result.css).toIncludeCss(css`
      .peer:disabled:focus:hover ~ .peer-disabled\:peer-focus\:peer-hover\:border-blue-500 {
        border-color: #3b82f6;
      }
    `)
  ...
  it('including just the base layer should not produce variants', () => {
    ...
    return run('@segmentdesign base', config).then((result) => {
  ...
  it('variants for utilities should not be produced in a file without a utilities layer', () => {
    ...
    return run('@segmentdesign components', config).then((result) => {
      return expect(result.css).toMatchFormattedCss(css`
        @media (min-width: 640px) {
          .sm\:container {
            width: 100%;
  ...
  it('appends variants to the correct place when using postcss documents', () => {
    ...
    doc.append(postcss.parse(`@segmentdesign base`))
    doc.append(postcss.parse(`@segmentdesign utilities`))
