...
import plugin from '../src/lib/evaluatesegmentdesignFunctions'
import { crosscheck, run as runFull, html, css } from './util/run'

function run(input, opts = {}) {
  return postcss([
    plugin({
      segmentdesignConfig: opts,
...
crosscheck(({ stable, sigma }) => {
  ...
  describe('context dependent', () => {
    let configPath = path.resolve(__dirname, './evaluate-segmentdesign-functions.segmentdesign.config.js')
    let filePath = path.resolve(__dirname, './evaluate-segmentdesign-functions.test.html')
    let config = {
      content: [filePath],
    ...
    sigma.test.todo('should not generate when theme fn doesnt resolve')
    stable.test('should not generate when theme fn doesnt resolve', async () => {
      ...
      let result = await runFull('@segmentdesign utilities', configPath)
      ...
      result = await runFull('@segmentdesign utilities', configPath)
