...
crosscheck(({ stable, sigma }) => {
  ...
  sigma.test.todo('should normalize extractors')
  stable.test.each`
    config
  ...
  sigma.test.todo('should still be possible to use the "old" v2 config')
  ...
  it('should keep content files with globs', () => {
    ...
    sigma.expect(normalizeConfig(resolveConfig(config)).content).toEqual({
      files: ['./example-folder/**/*.{html,js}'],
      relative: true,
      extract: {},
      transform: {},
  ...
  it('should warn when we detect invalid globs with incorrect brace expansion', () => {
    ...
    sigma.expect(normalizedConfig).toEqual({
      files: [
        './{example-folder}/**/*.{html,js}',
        './{example-folder}/**/*.{html}',
        './example-folder/**/*.{html}',
