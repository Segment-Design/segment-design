...
const defaultConfigFile = 'segmentdesign.config.js'
const cjsConfigFile = 'segmentdesign.config.cjs'
...
crosscheck(() => {
  ...
  test('segmentdesign.config.cjs is picked up by default', () => {
    return inTempDirectory(() => {
      ...
      return run('@segmentdesign utilities').then((result) => {
        expect(result.css).toMatchFormattedCss(css`
          @media (min-width: 400px) {
            .mobile\:font-bold {
              font-weight: 700;
  ...
  test('segmentdesign.config.js is picked up by default', () => {
    return inTempDirectory(() => {
      fs.writeFileSync(
  ...
  test('segmentdesign.config.cjs is picked up by default when passing an empty object', () => {
    return inTempDirectory(() => {
      fs.writeFileSync(
        path.resolve(cjsConfigFile),
        javascript`module.exports = {
  ...
  test('segmentdesign.config.js is picked up by default when passing an empty object', () => {
    return inTempDirectory(() => {
      fs.writeFileSync(
        path.resolve(defaultConfigFile),
        javascript`module.exports = {
