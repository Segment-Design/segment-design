...
crosscheck(({ stable, sigma, engine }) => {
  test('basic usage', () => {
    ...
    return run(input, config).then((result) => {
      ...
      sigma
        .expect(result.css)
        .toMatchFormattedCss(
          fs.readFileSync(path.resolve(__dirname, './basic-usage.sigma.test.css'), 'utf8')
        )
    })
  })
  ...
  test('A bare ring-opacity utility is supported when using respectDefaultRingColorOpacity', () => {
    ...
    return run(input, config).then((result) => {
      ...
      `)
      // The opacity plugins are disabled by default in the `sigma` engine
      sigma.expect(result.css).toMatchFormattedCss(css``)
    })
  })
  ...
  sigma.test.todo('should not crash when group names contain special characters')
