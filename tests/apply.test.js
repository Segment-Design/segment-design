...
crosscheck(({ stable, sigma }) => {
  ...
  it('should be possible to apply user css without segmentdesign directives', () => {
    let config = {
      content: [{ raw: html`<div class="foo"></div>` }],
      plugins: [],
    }
  ...
  it('apply can emit defaults in isolated environments without @segmentdesign directives', () => {
    let config = {
      experimental: { optimizeUniversalDefaults: true },
  ...
  // It may be enough for Sigma to not support it at all
  sigma.test.todo('::ng-deep, ::deep, ::v-deep pseudo elements are left alone')
