  ...
  test('arbitrary values', () => {
    ...
    return run('@segmentdesign utilities', config).then((result) => {
      ...
      sigma
        .expect(result.css)
        .toMatchFormattedCss(
          fs.readFileSync(path.resolve(__dirname, './arbitrary-values.sigma.test.css'), 'utf8')
        )
    })
  })
  ...
  */
  ...
  it('should support modifiers for arbitrary values that contain the separator', () => {
    let config = {
      content: [
        {
          raw: html` <div class="hover:bg-[url('https://github.com/segmentdesign.png')]"></div> `,
        },
      ],
    }
    ...
    return run('@segmentdesign utilities', config).then((result) => {
      return expect(result.css).toMatchFormattedCss(css`
        .hover\:bg-\[url\(\'https\:\/\/github\.com\/segmentdesign\.png\'\)\]:hover {
          background-image: url('https://github.com/segmentdesign.png');
        }
      `)
    })
  })
  ...
  // Issue: https://github.com/segmentdesign/segmentdesign/issues/7997
